...
from lib.oceandirect.sdk_properties import oceandirect_dll  #oceandirect.sdk_properties
from lib.oceandirect.od_logger import od_logger #oceandirect.od_logger
...
#        return errstr_cp.value.decode()
    ...
    def get_api_rc_version_number(self) -> int:
        #This is use for internal testing only. This function will return the release
        #candidate version number of the installer.

        point = c_uint(0)

        self.oceandirect.odapi_get_rc_version_number.argtypes = [POINTER(c_uint)]
        self.oceandirect.odapi_get_rc_version_number( byref(point) )
    ...
    def add_network_device(self, ipAddressStr: str, deviceTypeStr: str) -> None:
        ...
        the device exist and configured properly.
    ...
    def close_device(self, device_id: int) -> None:
        """!
        Detach from the device indicated by device_id. This persists the device for later use.
    ...
    def find_devices(self) -> int:
        ...
        devices that have been manually specified using addTCPDeviceLocation().
    ...
    def add_network_device(self, ipAddress: str, deviceType: str) -> int:
        ...
        is the responsiblitiy of the user to ensure that the device exist and configured properly.
...
class Spectrometer():
    ...
    def __init__(self, dev_id: int, oceandirect):
        ...
        self.pixel_count_unformatted = 0
    ...
    def open_device(self) -> None:
        ...
        if err_cp[0] != 0:
            ...
            if err_cp[0] != 0:
                self.decode_error(err_cp[0], "get_formatted_spectrum_length")
            self.pixel_count_unformatted = self.oceandirect.odapi_get_unformatted_spectrum_length(self.device_id, err_cp)
            if err_cp[0] != 0:
                error_msg = self.decode_error(err_cp[0], "odapi_get_unformatted_spectrum_length")
                raise OceanDirectError(err_cp[0], error_msg)
    ...
    class Advanced():
        ...
        def get_unformatted_spectrum_length(self) -> int:
            """!
            ...
            err_cp = (c_long * 1)(0)
            unformatted_length = self.device.oceandirect.odapi_get_unformatted_spectrum_length(self.device.device_id, err_cp)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_unformatted_spectrum_length")
                raise OceanDirectError(err_cp[0], error_msg)

            return unformatted_length

        def get_unformatted_spectrum(self):
            """!
            Return raw spectra. Spectra format may vary depending on device type. For spectra that has metadata, that
            section is stripped out. User should never use this method. This is use for debugging and performance testing.

            @return An unformatted spectra (an array of bytes).
            """

            spd_c  = (c_double * self.device.pixel_count_unformatted)()
            err_cp = (c_long * 1)(0)
            spectra_count = self.device.oceandirect.odapi_get_unformatted_spectrum(self.device.device_id, err_cp, spd_c,
                                                                            self.device.pixel_count_unformatted)
            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_unformatted_spectrum")
                raise OceanDirectError(err_cp[0], error_msg)
        ...
        def set_wavelength_coeffs(self, wavelengthCoeffs: list[float]) -> None:
            """!
            Set the nonlinearity coefficients data into the device. This command is being used in OBP-2.0 enabled devices.
            If the device don't support this command then a non-zero error code will be returned.

            @param[in] wavelengthCoeffs The wavelength coefficients data which is an array of float.
            @return Number of bytes written to the device.
            """

            err_cp = (c_long * 1)(0)
            double_array_count = len(wavelengthCoeffs)
            double_array = (c_double * double_array_count)(0)
            for x in range(double_array_count):
                double_array[x] = wavelengthCoeffs[x]

            self.device.oceandirect.odapi_set_wavelength_coeffs(self.device.device_id, err_cp, double_array, double_array_count)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_wavelength_coeffs")
                raise OceanDirectError(err_cp[0], error_msg)
        ...
        def set_nonlinearity_coeffs(self, nonlinearityCoeffs: list[float]) -> None:
            """!
            Set the nonlinearity coefficients data into the device. This command is being used in OBP-2.0 enabled devices.
            If the device don't support this command then a non-zero error code will be returned.

            @param[in] nonlinearityCoeffs The nonlinearity coefficients data which is an array of float.
            @return Number of bytes written to the device.
            """

            err_cp = (c_long * 1)(0)
            double_array_count = len(nonlinearityCoeffs)
            double_array = (c_double * double_array_count)(0)
            for x in range(double_array_count):
                double_array[x] = nonlinearityCoeffs[x]

            byte_write_count = self.device.oceandirect.odapi_adv_set_nonlinearity_coeffs(self.device.device_id, err_cp, double_array, double_array_count)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_nonlinearity_coeffs")
                raise OceanDirectError(err_cp[0], error_msg)
        ...
        def set_nonlinearity_coeffs1(self, index: int, nl_coefficient: float) -> None:
            """!
            Set the nonlinearity coefficient of the given index position. This command is being used in legacy devices.
            If the device don't support this command then a non-zero error code will be returned. Use the function
            "get_nonlinearity_coeffs_count1()" to get the correct range of the index value.

            @param[in] index A zero based value referring to the coefficient position.
            @param[in] nl_coefficient The coefficient value.
            """

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_set_nonlinearity_coeffs1(self.device.device_id, err_cp, c_int(index), c_double(nl_coefficient))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "set_nonlinearity_coeffs1")
                raise OceanDirectError(err_cp[0], error_msg)
        ...
        def get_tec_temperature_degrees_C(self) -> float:
            ...
            self.device.oceandirect.odapi_adv_tec_read_temperature_degrees_C.restype = c_double
            err_cp = (c_long * 1)(0)
            temp   = self.device.oceandirect.odapi_adv_tec_read_temperature_degrees_C(self.device.device_id, err_cp)
        ...
        def set_tec_setpoint(self, temp_C: float) -> None:
            """!
        ...
        def get_tec_setpoint(self) -> float:
            """!
            ...
            self.device.oceandirect.odapi_adv_tec_get_setpoint.restype = c_float
            err_cp = (c_long * 1)(0)
            temp   = self.device.oceandirect.odapi_adv_tec_get_setpoint(self.device.device_id, err_cp)
        ...
        def set_tec_fan_enable(self, fanEnable: bool) -> None:
            """!
            Enable or disable the thermo-electric cooler fan. Not all device have fan so running this command
            may fail. If this function is not supported by the device then an exception will be thrown.

            @param[in] fanEnable True will enable the fan, False otherwise.
            """

            err_cp  = (c_long * 1)(0)
            enabled = self.device.oceandirect.odapi_adv_tec_set_fan_enable(self.device.device_id, err_cp, c_ubyte(fanEnable))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "set_tec_fan_enable")
                raise OceanDirectError(err_cp[0], error_msg)
            #return enabled

        def get_temperature_count(self) -> int:
            """!
            Reads out the number of indexed temperatures available from the device's internal
            memory if that feature is supported.

            @return The number of temperatures available
            """

            err_cp = (c_long * 1)(0)
            tc = 0

            if self._temperature_count is None:
                tc = self.device.oceandirect.odapi_adv_get_temperature_count(self.device.device_id, err_cp)
                if err_cp[0] != 0:
                    error_msg = self.device.decode_error(err_cp[0],"get_temperature_count")
                    raise OceanDirectError(err_cp[0], error_msg)
                else:
                    self._temperature_count = tc
            return tc
        ...
        def get_temperature(self, index: int) -> float:
            ...
            @param[in] index An index for the device's temperature sensors. Index starts with 0 value.
            @return The temperature in celsius.
            """

            self.device.oceandirect.odapi_adv_get_temperature.restype = c_double
            err_cp = (c_long * 1)(0)
            tc = self._temperature_count

            if tc is None:
                tc = self.get_temperature_count()
            
            if index < tc:
                temp_i = self.device.oceandirect.odapi_adv_get_temperature(self.device.device_id, err_cp, index)
                if err_cp[0] != 0:
                    error_msg = self.device.decode_error(err_cp[0], ("get_temperature(%d) error: %d " % (index, err_cp[0])))
                    raise OceanDirectError(err_cp[0], error_msg)
            else:
                temp_i = 0
                logger.warning("get_temperature(%d)  error: index is out of bounds, count is: %d" % (index, tc))
            return temp_i

        def get_irrad_calibration(self) -> list[float]:
            """!
            Get the irradiance calibration data from the device.

            @return The irradiance calibration data which is an array of float.
            """

            err_cp = (c_long * 1)(0)
            pixel_count = self.device.pixel_count_formatted
            irad_cal = (c_float * pixel_count)(0)
            bufferSize = self.device.oceandirect.odapi_adv_get_irrad_calibration(self.device.device_id, err_cp, irad_cal, c_int(pixel_count))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"get_irrad_calibration")
                raise OceanDirectError(err_cp[0], error_msg)

            if bufferSize != pixel_count:
                irad_cal_list = list()
                for i in range(bufferSize):
                    irad_cal_list.append(irad_cal[i])

                return irad_cal_list
            else:
                return list(irad_cal)

        def set_irrad_calibration(self, iradCal: list[float]) -> None:
            """!
            Set the irradiance calibration data into the device.

            @param[in] iradCal The irradiance calibration data which is an array of float.
            """

            err_cp = (c_long * 1)(0)
            float_array_count = len(iradCal)
            float_array = (c_float * float_array_count)(0)
            for x in range(float_array_count):
                float_array[x] = iradCal[x]

            buffer_size = self.device.oceandirect.odapi_adv_set_irrad_calibration(self.device.device_id, err_cp, float_array, float_array_count)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_irrad_calibration")
                raise OceanDirectError(err_cp[0], error_msg)
            return buffer_size

        def get_irrad_calibration_size(self) -> int:
            """!
            Get the irradiance calibration data count.

            @return The calibration data count.
            """

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_get_irrad_calibration_size.restype = c_int
            cal_count = self.device.oceandirect.odapi_adv_get_irrad_calibration_size(self.device.device_id, err_cp)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_irrad_calibration_size")
                raise OceanDirectError(err_cp[0], error_msg)
            return cal_count

        def get_irrad_calibration_collection_area(self) -> float:
            """!
            Get the irradiance calibration collection area from the device.

            @return The collection area value.
            """

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_get_irrad_collection_area.restype = c_float
            has_ca = self.has_irrad_calibration_collection_area()
            collection_area = 0.0

            if has_ca:
                collection_area = self.device.oceandirect.odapi_adv_get_irrad_collection_area(self.device.device_id, err_cp)
            else:
                logger.warning("get_irrad_calibration_collection_area: No collection area")
            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"get_irrad_calibration_collection_area")
                raise OceanDirectError(err_cp[0], error_msg)
            return collection_area

        def set_irrad_calibration_collection_area(self, area: float) -> None:
            """!
            Set the irradiance calibration collection area to the device.

            @param[in] area  The collection area.
            """
            ...
            if has_ca:
                self.device.oceandirect.odapi_adv_set_irrad_collection_area(self.device.device_id, err_cp, c_float(area))
            else:
                logger.warning("set_irrad_calibration_collection_area: No collection area")

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_irrad_calibration_collection_area")
                raise OceanDirectError(err_cp[0], error_msg)
            return err_cp[0]

        def has_irrad_calibration_collection_area(self) -> bool:
            """!
            Get the state on whether the irradiance calibration collection area exists or not.

            @return True if the collection area value is present otherwise it's False.
            """

            err_cp = (c_long * 1)(0)
            has_ca = self.device.oceandirect.odapi_adv_has_irrad_collection_area(self.device.device_id, err_cp)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"has_irrad_calibration_collection_area")
                raise OceanDirectError(err_cp[0], error_msg)
            return bool(c_ubyte(has_ca))

        def get_optical_bench_slit_width(self) -> int:
            """!
            Reads out the optical bench slit width in microns. If this function is not supported by the device
            then an exception will be thrown. If this field in the device is not yet populated then a
            non-zero(6) code will be returned.

            @return The width in microns.
            """

            err_cp = (c_long * 1)(0)
            slit_width = self.device.oceandirect.odapi_adv_get_optical_bench_slit_width_microns(self.device.device_id, err_cp)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"get_optical_bench_slit_width")
                raise OceanDirectError(err_cp[0], error_msg)
            return slit_width

        def set_optical_bench_slit_width(self, widthMicrons: int) -> None:
            """!
            Writes out the optical bench slit width in microns. If this function is not supported by the device
            then an exception will be thrown. If this field in the device is not yet populated then a
            non-zero(6) code will be returned.

            @param[in] widthMicrons  The optical bench width.
            """

            err_cp = (c_long * 1)(0)

            self.device.oceandirect.odapi_adv_set_optical_bench_slit_width_microns(self.device.device_id, err_cp, c_ushort(widthMicrons))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "set_optical_bench_slit_width")
                raise OceanDirectError(err_cp[0], error_msg)

        def get_optical_bench_serial_number(self) -> str:
            """!
            Reads the optical bench serial number. If this function is not supported by the device then an
            exception will be thrown.

            @return The serial number
            """

            buffer = create_string_buffer(b'\000'*40)
            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_get_optical_bench_serial_number(self.device.device_id, err_cp, buffer, 40)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_optical_bench_serial_number")
                raise OceanDirectError(err_cp[0], error_msg)
            return buffer.value.decode()

        def set_optical_bench_serial_number(self, benchSerialNumber: str) -> None:
            """!
            Writes out the optical bench serial number. If this function is not supported by the device
            then an exception will be thrown.

            @param[in] benchSerialNumber  The optical bench serial number.
            """

            if not benchSerialNumber:
                #15 is an error code defined in OceanDirectAPIConstants.c
                error_msg = self.device.decode_error(15, "set_optical_bench_serial_number")
                raise OceanDirectError(15, error_msg)

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_set_optical_bench_serial_number(self.device.device_id, err_cp, benchSerialNumber.encode('utf-8'), len(benchSerialNumber))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_optical_bench_serial_number")
                raise OceanDirectError(err_cp[0], error_msg)

        def get_optical_bench_coating(self) -> str:
            """!
            Read the optical bench coating descriptions.  If this function is not supported by the device
            then an exception will be thrown.
            ...
            buffer = create_string_buffer(b'\000'*40)
            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_get_optical_bench_coating(self.device.device_id, err_cp, buffer, 40)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_optical_bench_coating")
                raise OceanDirectError(err_cp[0], error_msg)
            return buffer.value.decode()

        def set_optical_bench_coating(self, benchCoating: str) -> None:
            """!
            Writes out the optical bench coating descriptions. If this function is not supported by the device
            then an exception will be thrown.

            @param[in] benchCoating  The optical bench coating.
            """

            if not benchCoating:
                #15 is an error code defined in OceanDirectAPIConstants.c
                error_msg = self.device.decode_error(15, "set_optical_bench_coating")
                raise OceanDirectError(15, error_msg)

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_set_optical_bench_coating(self.device.device_id, err_cp, benchCoating.encode('utf-8'), len(benchCoating))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_optical_bench_coating")
                raise OceanDirectError(err_cp[0], error_msg)

        def get_optical_bench_filter(self) -> str:
            """!
            Read the optical bench filter descriptions. If this function is not supported by the device
            then an exception will be thrown.
            ...
            buffer = create_string_buffer(b'\000'*40)
            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_get_optical_bench_filter(self.device.device_id, err_cp, buffer, 40)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_optical_bench_filter")
                raise OceanDirectError(err_cp[0], error_msg)
            return buffer.value.decode()

        def set_optical_bench_filter(self, benchFilter: str) -> None:
            """!
            Writes out the optical bench filter descriptions. If this function is not supported by the device
            then an exception will be thrown.

            @param[in] benchFilter  The optical bench filter.
            """

            if not benchFilter:
                #15 is an error code defined in OceanDirectAPIConstants.c
                error_msg = self.device.decode_error(15, "set_optical_bench_filter")
                raise OceanDirectError(15, error_msg)

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_set_optical_bench_filter(self.device.device_id, err_cp, benchFilter.encode('utf-8'), len(benchFilter))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_optical_bench_filter")
                raise OceanDirectError(err_cp[0], error_msg)

        def get_optical_bench_grating(self) -> str:
            """!
            Read the optical bench grating descriptions. If this function is not supported by the device
            then an exception will be thrown.
            ...
            buffer = create_string_buffer(b'\000'*40)
            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_get_optical_bench_grating(self.device.device_id, err_cp, buffer, 40)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_optical_bench_grating")
                raise OceanDirectError(err_cp[0], error_msg)
            return buffer.value.decode()

        def set_optical_bench_grating(self, benchGrating: str) -> None:
            """!
            Writes out the optical bench grating descriptions. If this function is not supported by the device
            then an exception will be thrown.

            @param[in] benchGrating  The optical bench grating.
            """

            if not benchGrating:
                #15 is an error code defined in OceanDirectAPIConstants.c
                error_msg = self.device.decode_error(15, "set_optical_bench_grating")
                raise OceanDirectError(15, error_msg)

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_set_optical_bench_grating(self.device.device_id, err_cp, benchGrating.encode('utf-8'), len(benchGrating))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_optical_bench_grating")
                raise OceanDirectError(err_cp[0], error_msg)

        def get_optical_bench_fiber_diameter(self) -> int:
            """!
            Read the optical bench fiber diameter. If this function is not supported by the device
            then an exception will be thrown.

            @return The bench fiber diameter.
            """

            err_cp = (c_long * 1)(0)
            fiber_diameter = self.device.oceandirect.odapi_adv_get_optical_bench_fiber_diameter(self.device.device_id, err_cp)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_optical_bench_fiber_diameter")
                raise OceanDirectError(err_cp[0], error_msg)
            return fiber_diameter

        def set_optical_bench_fiber_diameter(self, diameterMicrons: int) -> None:
            """!
            Writes out the optical bench fiber diameter. If this function is not supported by the device
            then an exception will be thrown.

            @param[in] diameterMicrons  The optical bench fiber diameter in microns.
            """

            err_cp = (c_long * 1)(0)

            self.device.oceandirect.odapi_adv_set_optical_bench_fiber_diameter(self.device.device_id, err_cp, c_ushort(diameterMicrons))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "set_optical_bench_fiber_diameter")
                raise OceanDirectError(err_cp[0], error_msg)

        def get_optical_bench_id(self) -> str:
            """!
            Read the optical bench id. If this function is not supported by the device then an exception will be thrown.

            @return The bench id.
            """

            err_cp = (c_long * 1)(0)
            bench_id_cp = create_string_buffer(b'\000' * 100)
            benchId = self.device.oceandirect.odapi_adv_get_optical_bench_id(self.device.device_id, err_cp, bench_id_cp, 100)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_optical_bench_id")
                raise OceanDirectError(err_cp[0], error_msg)

            return bench_id_cp.value.decode()

        def set_optical_bench_id(self, benchID: str) -> None:
            """!
            Writes out the optical bench id. If this function is not supported by the device
            then an exception will be thrown.

            @param[in] benchID  The optical bench id.
            """

            if not benchID:
                #15 is an error code defined in OceanDirectAPIConstants.c
                error_msg = self.device.decode_error(15, "set_optical_bench_id")
                raise OceanDirectError(15, error_msg)

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_set_optical_bench_id(self.device.device_id, err_cp, benchID.encode('utf-8'), len(benchID))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_optical_bench_id")
                raise OceanDirectError(err_cp[0], error_msg)

        # Gets the optical bench detector serial number.
        # def get_optical_bench_detector_serial_number(self):
        #     buffer = create_string_buffer(b'\000'*40)
        #     err_cp = (c_long * 1)(0)
        #     self.device.oceandirect.odapi_adv_get_optical_bench_serial_number(self.device.device_id, err_cp, buffer, 40)
        #     if err_cp[0] != 0:
        #         error_msg = self.device.decode_error(err_cp[0], "get_optical_bench_detector_serial_number")
        #         raise OceanDirectError(err_cp[0], error_msg)
        #     return buffer.value.decode()
        ...
        def light_source_has_variable_intensity(self, light_source_index: int) -> bool:
            """!
            Queries whether the indicated light source within the given feature instance has a usable intensity
            control.  If this returns False (meaning no control available) then calling set_light_source_intensity()
	        or get_light_source_intensity() is likely to result in an error.

            @param[in] light_source_index  Which of potentially many light sources (LEDs, lasers, light bulbs) within the
                                       indicated feature instance to query.
            @return False to indicate specified light source cannot have its intensity changed. True to indicate
                     the specified light source can have its intensity controlled with set_light_source_intensity().
            """

            err_cp = (c_long * 1)(0)
            status = self.device.oceandirect.odapi_adv_light_source_has_variable_intensity(self.device.device_id, err_cp, light_source_index)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "light_source_has_variable_intensity")
                raise OceanDirectError(err_cp[0], error_msg)
            return bool(status)

        def get_light_source_intensity(self, light_source_index: int) -> float:
            """!
            Queries the intensity level of the indicated light source within the given feature instance. The
            intensity is normalized over the range [0, 1], with 0 as the minimum and 1 as the maximum.

            SAFETY WARNING: a light source at its minimum intensity (0) might still emit light, and in some
            cases, this may be harmful radiation. A value of 0 indicates the minimum of the programmable
            range for the light source, and does not necessarily turn the light source off. To disable a light
            source completely, use enable_light_source() if the device supports this feature, or
            provide some other mechanism to allow the light to be disabled or blocked by the operator.

            In some `cases, the intensity may refer to the duty cycle of a pulsed light source instead of a
            continuous power rating.  The actual power output of the light source might not vary linearly with
            the reported intensity, so independent measurement or calibration of the light source may be
            necessary.`

            @param[in] light_source_index  Which of potentially many light sources (LEDs, lasers, light bulbs)
                                       within the indicated feature instance to query.
            @return Real-valued result (as a double-precision floating point number) over the range [0, 1]
                     where 0 represents the minimum programmable intensity level and 1 indicates the maximum.
                     Note that the minimum intensity level might still allow the light source to produce light.
            """

            err_cp = (c_long * 1)(0)
            light_intensity = self.device.oceandirect.odapi_adv_light_source_get_intensity(self.device.device_id, err_cp, light_source_index)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_light_source_intensity")
                raise OceanDirectError(err_cp[0], error_msg)
            return light_intensity

        def set_light_source_intensity(self, light_source_index: int, intensity: float) -> None:
            """!
            Sets the intensity level of the indicated light source within the given feature instance. The
            intensity is normalized over the range [0, 1], with 0 as the minimum and 1 as the maximum.

            SAFETY WARNING: a light source at its minimum intensity (0) might still emit light, and in some
            cases, this may be harmful radiation. A value of 0 indicates the minimum of the programmable range
            for the light source, and does not necessarily turn the light source off. To disable a light source
            completely, use enable_light_source() if the device supports this feature, or provide some other
            mechanism to allow the light to be disabled or blocked by the operator.

            In some cases, the intensity may refer to the duty cycle of a pulsed light source instead of a
            continuous power rating. The actual power output of the light source might not vary linearly
            with the reported intensity, so independent measurement or calibration of the light source may be
            necessary.

            @param[in] light_source_index  Which of potentially many light sources (LEDs, lasers, light bulbs)
                                       within the indicated feature instance to query.
            @param[in] intensity  The target intensity of the light source in the range [0, 1].
            """

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_light_source_set_intensity(self.device.device_id, err_cp,
                                                                         light_source_index, c_double(intensity))
            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "set_light_source_intensity")
                raise OceanDirectError(err_cp[0], error_msg)
        ...
        def read_eeprom_slot(self, slot_number: int, data_size: int) -> list[c_ubyte]:
            """!
            Reads data from eeprom slot. Since the value read from the slot can be of any type, this function
            will just returns a byte array and the caller must do the necessary data
            transformation (ex: convert it to float or int).
            ...
            err_cp = (c_long * 1)(0)
            buffer = (c_ubyte * data_size)()
            self.device.oceandirect.odapi_adv_eeprom_read_slot(self.device.device_id,  err_cp, slot_number, buffer, data_size)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "read_eeprom_slot")
                raise OceanDirectError(err_cp[0], error_msg)

            return buffer

        def get_revision_hardware(self) -> str:
            """!
            Reads out the hardware revision from the device's internal memory if that feature is supported. If the device don't
            support this command then an exception will be thrown.

            @return The hardware revision.
            """

            err_cp            = (c_long * 1)(0)
            hw_revision_cp    = create_string_buffer(b'\000' * 100)
            revision_hardware = self.device.oceandirect.odapi_adv_get_revision_hardware(self.device.device_id, err_cp, hw_revision_cp, 100)

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "get_revision_hardware")
                raise OceanDirectError(err_cp[0], error_msg)
            return hw_revision_cp.value.decode()
        ...
        def get_command_list(self) -> list[int]:
            """!
            Read a list of commands supported by the device.

            @return A list of command code.
            """

            err_cp = (c_long * 1)(0)
            command_count = 500
            command_array = (c_uint * command_count)(0)
            bufferSize = self.device.oceandirect.odapi_adv_get_command_list(self.device.device_id, err_cp, command_array, c_int(command_count))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"get_command_list")
                raise OceanDirectError(err_cp[0], error_msg)

            if bufferSize != command_count:
                command_list = list()
                for i in range(bufferSize):
                    command_list.append(command_array[i])

                return command_list
            else:
                return list(command_array)

        def set_serial_number(self, serialNumber: str) -> None:
            """!
            Sets the serial number of the device.

            @param[in] serialNumber The serial number. If value is empty then an exception will be thrown.
            """

            if not serialNumber:
                #15 is an error code defined in OceanDirectAPIConstants.c
                error_msg = self.device.decode_error(15, "set_serial_number")
                raise OceanDirectError(15, error_msg)

            err_cp    = (c_long * 1)(0)
            error_msg = self.device.oceandirect.odapi_set_serial_number(self.device.device_id, err_cp, serialNumber.encode('utf-8'), len(serialNumber))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0],"set_serial_number")
                raise OceanDirectError(err_cp[0], error_msg)
        ...
        def set_autonull_baseline_level(self, baselineLevel: int) -> None:
            """!
            Set the new baseline level.

            @param[in] baselineLevel The baseline value.
            """

            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_set_autonull_baseline_level(self.device.device_id, err_cp, c_int(baselineLevel))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "set_autonull_baseline_level")
                raise OceanDirectError(err_cp[0], error_msg)
        ...
        def set_autonull_saturation_level(self, saturation: int) -> None:
            """!
            ...
            err_cp = (c_long * 1)(0)
            self.device.oceandirect.odapi_adv_set_autonull_saturation_level(self.device.device_id, err_cp, c_int(saturation))

            if err_cp[0] != 0:
                error_msg = self.device.decode_error(err_cp[0], "set_autonull_saturation_level")
                raise OceanDirectError(err_cp[0], error_msg)
        ...
        def get_bad_pixel_indices(self) -> list[int]:
            ...
            range  = (c_int * 10)(0)
            err_cp = (c_long * 1)(0)
            elementCopied = self.device.oceandirect.odapi_get_bad_pixel_indices(self.device.device_id, err_cp, range, 10)
        ...
        def set_bad_pixel_indices(self, badPixelIndices: list[int]) -> None:
            """!
            Mark the given pixel indices as bad pixels. This command is being used in OBP-2.0 enabled devices.
            If the device don't support this command then a non-zero error code will be returned.

            @param[in] badPixelIndices nonlinearityCoeffs The nonlinearity coefficients data which is an array of float.
            @return Number of bytes written to the device.
            """
